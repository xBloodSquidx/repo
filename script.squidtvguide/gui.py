...
dexterUsername = addon.getSetting("dexter.username")
dexterPassword = addon.getSetting("dexter.password")
playerFormat = addon.getSetting("player.format")
...
class TVGuide(xbmcgui.WindowXML):
    ...
    def __new__(cls):
        return super(TVGuide, cls).__new__(cls, 'script.squidtvguide-main.xml', ADDON.getAddonInfo('path'))
    ...
    def onActionTVMode(self, action):
        ...
        elif action.getId() in [ACTION_PARENT_DIR, KEY_NAV_BACK, KEY_CONTEXT_MENU, ACTION_SELECT_ITEM, ACTION_PREVIOUS_MENU]:
            self.viewStartDate = datetime.datetime.today()
    ...
    def onActionEPGMode(self, action):
        ...
        if action.getId() == ACTION_LEFT:
            ...
            self.viewStartDate -= datetime.timedelta(minutes=self.viewStartDate.minute % 30,
                                                     seconds=self.viewStartDate.second)
            self.onRedrawEPG(self.channelIdx, self.viewStartDate)
        elif action.getId() in [ACTION_SHOW_INFO] and controlInFocus is not None:
    ...
    def playChannel(self, channel):
        ...
        if wasPlaying == True:
            currentlyPlaying = self.player.getPlayingFile()

        if url:

            if url[0:19] == 'http://158.69.54.54':
                url = url.replace("USERNAME", "%s" % dexterUsername ) 
                url = url.replace("PASSWORD", "%s" % dexterPassword ) 
                url = url.replace("FORMAT", "%s" % playerFormat ) 

            if (wasPlaying == True and currentlyPlaying == url):
...
class PopupMenu(xbmcgui.WindowXMLDialog):
    ...
    def __new__(cls, database, program, showRemind):
        return super(PopupMenu, cls).__new__(cls, 'script.squidtvguide-menu.xml', ADDON.getAddonInfo('path'))
...
class ChannelsMenu(xbmcgui.WindowXMLDialog):
    ...
    def __new__(cls, database):
        return super(ChannelsMenu, cls).__new__(cls, 'script.squidtvguide-channels.xml', ADDON.getAddonInfo('path'))
...
class StreamSetupDialog(xbmcgui.WindowXMLDialog):
    ...
    def __new__(cls, database, channel):
        return super(StreamSetupDialog, cls).__new__(cls, 'script.squidtvguide-streamsetup.xml', ADDON.getAddonInfo('path'))
...
class ChooseStreamAddonDialog(xbmcgui.WindowXMLDialog):
    ...
    def __new__(cls, addons):
        return super(ChooseStreamAddonDialog, cls).__new__(cls, 'script.squidtvguide-streamaddon.xml',
