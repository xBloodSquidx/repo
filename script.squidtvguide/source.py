...
SETTINGS_TO_CHECK = ['xmltv.file', 'xmltv.logo.folder']
...
class Database(object):
        ...
        while True:
            ...
            try:
                ...
                if cancel_requested_callback is None:
                    xbmc.log('[script.squidtvguide] Database is locked, bailing out...', xbmc.LOGDEBUG)
                    break
                else:  # ignore 'database is locked'
                    xbmc.log('[script.squidtvguide] Database is locked, retrying...', xbmc.LOGDEBUG)

            except sqlite3.DatabaseError:
                self.conn = None
                if self.alreadyTriedUnlinking:
                    xbmc.log('[script.squidtvguide] Database is broken and unlink() failed', xbmc.LOGDEBUG)
                    break
                else:
                    try:
    ...
    def _updateChannelAndProgramListCaches(self, date, progress_callback, clearExistingProgramList):
        ...
        c = self.conn.cursor()
        try:
            xbmc.log('[script.squidtvguide] Updating caches...', xbmc.LOGDEBUG)
...
class XMLTVSource(Source):
    ...
    def parseXMLTVDate(self, origDateString):
        if origDateString.find(' ') != -1:
            ...
            xbmc.log('[script.squidtvguidetecbox] Date to normalize: ' + dateString, xbmc.LOGDEBUG)
            try:
            ...
            t = t + td_local
            xbmc.log('[script.squidtvguidetecbox] Import Time adjusted from: ' + str(t_tmp) + ' to: ' + str(t), xbmc.LOGDEBUG)
            return t
